...
from src.ConvGRU.utils import ConvGRULoader
import sys
...
class ConvGRUTrainer:
	def __init__(self, data_path, epoch, drop_out, lr, model_name, log_name, batch_size, n_classes, struct_num):
		...
		self.criterion = nn.CrossEntropyLoss().to(device=device)
		self.optimizer = optim.Adam(self.model.parameters(), lr, betas=(0.9, 0.999), eps=1e-8)
		...
		train_set = ConvGRULoader(train_data, train_labels, n_classes)
		vaild_set = ConvGRULoader(test_data, test_labels, n_classes)
